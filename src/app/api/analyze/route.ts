...
function buildResendAlertContent(input: {
  ...
  const html = `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; color: #1c1a18;">
      ...
      ${findingList}
      <p style="margin-top: 16px; font-size: 12px; color: #6a5f55;">This alert was generated by GeminEYE with Gemini-first analysis and Featherless gap-filling for anything Gemini may miss, plus Speechmatics intake and Resend alerting. Review the full report in the dashboard for details.</p>
    </div>
  `;

  const text = `GeminEYE risk alert\n\nContract: ${input.contractTitle}\nScore: ${input.score.toFixed(1)} / 10 (${riskLabel})\nThreshold: ${input.threshold.toFixed(1)} / 10\nProvider: ${input.providerLabel || "configured AI provider"}${input.model ? ` (${input.model})` : ""}\nStack: Gemini-first analysis with Featherless gap-filling, Speechmatics intake, Resend alerts\n\nSummary:\n${summaryItems.join("\n")}\n\nTop findings:\n${findingItems.map((item) => `${item.category} (${item.risk}) - ${item.recommendation}`).join("\n")}`;
